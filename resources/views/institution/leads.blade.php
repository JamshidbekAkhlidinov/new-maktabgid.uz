@php
    ...
    $tabs = [
        ['key' => 'all', 'label' => 'Hammasi', 'count' => $leadStats['total']],
        ['key' => 'pending', 'label' => 'Yangi', 'count' => $leadStats['pending']],
        ['key' => 'confirmed', 'label' => 'Tasdiqlangan', 'count' => $leadStats['confirmed']],
        ['key' => 'rejected', 'label' => 'Rad etilgan', 'count' => $leadStats['rejected']],
    ];
@endphp
...
>
    ...
    <div class="idash-lead-toolbar">
        ...
        <label class="idash-lead-search">
            ...
            <input type="text" class="js-filter-search" placeholder="Ism yoki izoh bo'yicha…" />
        </label>
    </div>
        ...
        @else
            <div class="idash-lead-table">
                ...
                @foreach ($leads as $lead)
                    @php
                        $childBits = array_filter([$lead->child_name, $lead->child_age ? "{$lead->child_age} yosh" : ($lead->target_grade ?: $lead->current_grade)]);
                    @endphp
                    <div class="idash-lead-row js-filter-row" data-app-id="{{ $lead->id }}" data-status="{{ $lead->status }}" data-search="{{ strtolower($lead->parent_name.' '.($lead->note ?? '')) }}">
                        ...
                        <div class="idash-lead-actions">
                            ...
                            @else
                                <a class="idash-lead-iconbtn" href="tel:{{ $lead->parent_phone }}" title="Qo'ng'iroq qilish">
