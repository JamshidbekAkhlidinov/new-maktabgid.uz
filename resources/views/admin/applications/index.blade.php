...
@section('content')
    ...
    <form method="GET" class="flex flex-wrap gap-2 mb-4">
        ...
        <select name="status" class="rounded-lg border border-slate-300 px-3.5 py-2 text-sm bg-white">
            ...
            <option value="completed" @selected(request('status') === 'completed')>Bo'lib o'tgan</option>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($applications as $app)
                    <tr>
                        ...
                        <td class="px-4 py-3">
                            @php($cls = ['pending' => 'bg-amber-50 text-amber-700', 'confirmed' => 'bg-emerald-50 text-emerald-700', 'completed' => 'bg-teal-50 text-teal-700', 'rejected' => 'bg-rose-50 text-rose-700'][$app->status] ?? 'bg-slate-100 text-slate-600')
